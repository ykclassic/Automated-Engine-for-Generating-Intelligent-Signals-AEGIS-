...
def update_all_data(config_path: str = "config/settings.yaml", timeframe: Optional[str] = None) -> Dict[str, DataFetchResult]:
    """
    Update all market data for configured assets and timeframes.
    ...
    Returns:
        Dictionary of results keyed by symbol_timeframe
    """
    logger.info("🚀 Starting AEGIS data update cycle")
    pipeline = DataPipeline(config_path)
    results = pipeline.run_pipeline(timeframe)
